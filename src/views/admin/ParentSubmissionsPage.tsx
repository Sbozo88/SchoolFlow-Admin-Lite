...
export function ParentSubmissionsPage() {
  ...
  async function handleMarkReviewed(submission: ParentSubmission) {
    try {
      setActionError("");
  ...
  async function handleConvertToLearner(submission: ParentSubmission) {
    try {
      setActionError("");
  ...
  const columns: TableColumn<ParentSubmission>[] = [
    {
      key: "date",
      header: "Date",
